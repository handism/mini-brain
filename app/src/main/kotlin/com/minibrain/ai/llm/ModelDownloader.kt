...
class ModelDownloader(private val context: Context) {
    ...
    private fun calculateSha256(file: File): String = try {
        ...
        Log.e(TAG, "Failed to calculate SHA-256 for ${file.name}", e)
        ""
    }
                    ...
                    when {
                        result is DownloadResult.Done -> {
                            if (verifyHash(tempFile, EMBEDDER_SHA256)) {
                                ...
                                Log.e(TAG, "Embedder hash verification failed")
                                tempFile.delete()
                    ...
                    when {
                        result is DownloadResult.Done -> {
                            if (verifyHash(tempFile, TOKENIZER_SHA256)) {
                                ...
                                Log.e(TAG, "Tokenizer hash verification failed")
                                tempFile.delete()
                    ...
                    when {
                        result is DownloadResult.Done -> {
                            if (verifyHash(tempFile, LLM_SHA256)) {
                                ...
                                Log.e(TAG, "LLM hash verification failed")
                                tempFile.delete()
